...
namespace slvr {
    ...
    void Solver::execute(const Task& task, std::vector<Task>& newTasks) {
        ...
        for (const Location& location : locations) {
            ...
            for (int i : result) {
                //Copy the Task to a new Task, place the piece, and add it to our list
                Task temp = task;
                placePiece(piece, i, location, &temp); 
                ///newTasks.push_back(std::move(temp));
                newTasks.push_back(temp);
            }
        }
        return;
    }
    ...
    void Solver::execute(const Task& task, std::vector<Solution>& newSolutions) {
        ...
        for (const Location& location : locations) {
            ...
            for (int i : result) {
                ...
                newSolutions.push_back(solution);
            }
        }
        return;
    }
            ...
            */
            ...
            if (self->useBatching) {
                ...
                for (int i = 0; i < numTasksToMove; ++i) {
                    ...
                    tasksToProcess.push_back(self->taskQueue.back());
                    ///self->taskQueue.pop_front();
                    self->taskQueue.pop_back();
                }
            } else {
                ///task = std::move(self->taskQueue.front());
                task = self->taskQueue.back();
                ///self->taskQueue.pop_front();
                self->taskQueue.pop_back();
            }
            ...
            self->taskQueue.insert(self->taskQueue.end(), newTasks.begin(), newTasks.end());
